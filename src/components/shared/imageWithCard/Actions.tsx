...
const Actions = ({ button, single, index }: ActionsProps) => {
  ...
  return (
    <LocaleLink
      ...
      target={!!button?.newTab}
      className={`flex h-9 items-center justify-center ${isDark ? 'bg-gray-800 text-gray-50' : 'bg-transparent text-gray-800'} cursor-pointer rounded-full border-2 border-gray-800 px-5 py-1 text-sm font-medium md:h-10.5 lg:h-9.5 xl:h-12.5 xl:text-base 2xl:border-3 ${
        single
