    ...
    >
      ...
      <span
        className={`absolute h-0.5 rounded-xl bg-gray-800 transition-all duration-300 ease-in-out ${
          isOpen ? 'w-[22px] rotate-45' : 'w-[22px] -translate-y-1.5 rotate-0'
        }`}
      />
      {/* Middle Line - becomes other part of X */}
      <span
        className={`absolute h-0.5 rounded-xl bg-gray-800 transition-all duration-300 ease-in-out ${
          isOpen ? 'w-[22px] -rotate-45' : 'w-[16px] translate-y-0 rotate-0'
        }`}
      />
      {/* Bottom Line - disappears when open */}
      <span
        className={`absolute h-0.5 rounded-xl bg-gray-800 transition-all duration-300 ease-in-out ${
          isOpen ? 'w-0 opacity-0' : 'w-[10px] translate-y-1.5'
        }`}
      />
    </button>
  );
